    ...
    private static func convertToAPIError(_ error: Error) -> Error {
        ...
        if let sessionTaskError = error as? SessionTaskError {
            ...
            case .connectionError:
                ...
                if let urlError = (try? (sessionTaskError as NSError).userInfo[NSUnderlyingErrorKey] as? URLError),
                   urlError.code == .timedOut {
                    return APIError(domain: "tonny.studymate-ios.api", statusCode: .gatewayTimeout, originalError: error)
                }
                return APIError(domain: "tonny.studymate-ios.api", statusCode: .unknown, originalError: error)
            case .requestError(let requestError):
                // リクエストエラーのメッセージを取得
                return APIError(domain: "tonny.studymate-ios.api", statusCode: .badRequest, responseObject: requestError.localizedDescription, originalError: error)
            case .responseError(let responseError):
                ...
                return APIError(domain: "tonny.studymate-ios.api", statusCode: StatusCode(code: statusCode), originalError: error)
            default:
                return APIError(domain: "tonny.studymate-ios.api", statusCode: .unknown, originalError: error)
            }
        }
        ...
        if let decodingError = error as? DecodingError {
            return APIError(domain: "tonny.studymate-ios.api", statusCode: .badRequest, responseObject: decodingError.localizedDescription, originalError: decodingError)
        }
        // それ以外のエラー
        return APIError(domain: "tonny.studymate-ios.api", statusCode: .unknown, originalError: error)
    }
